...
# best scoring design from the NSGA-II run (analysis/ga_run.log, seed 1)
BEST = dict(material="LNGT44", circuit="potcore", n_gon=8, r_face=19.4e-3,
...
def run(label, kw, do_dynamics=True):
    d = Design(**kw)
    print("\n" + "=" * 86)
    print(f"{label}")
    print("=" * 86)
    ...
    print(f"  4 MECHANICS  hold {mech['hold_ratio']:.1f} x weight,  "
          f"pivot work/barrier {mech['pivot_ratio']:.0f},  "
...
print("=" * 86)
print("FULL-FIDELITY VERIFICATION OF THE OPTIMISED DESIGN")
print("=" * 86)
run("BASELINE  (as built: Alnico 5, bare rod, 18 faces)", BASELINE)
run("OPTIMISED (NSGA-II best, seed 1)", BEST)
